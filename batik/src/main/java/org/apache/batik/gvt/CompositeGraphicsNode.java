 ...
 * @version $Id: CompositeGraphicsNode.java,v 1.1 2005/11/21 09:51:17 dev Exp $
 */
...
//        System.out.println("CompositeG\t"+geometryBounds);
...
//             System.err.println("geometryBounds are null");
            ...
            while(geometryBounds == null && i < count){
                geometryBounds = 
		    children[i++].getTransformedGeometryBounds (aff);
            }

            Rectangle2D cgb = null;
            while (i<count) {
                cgb = children[i++].getTransformedGeometryBounds(aff);
                if (cgb != null) {
                    if (geometryBounds == null) {
                        // another thread has set the geometry bounds to null,
                        // need to recall this function
