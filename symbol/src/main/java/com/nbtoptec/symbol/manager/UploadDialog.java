...
public class UploadDialog extends TitledDialog{
	...
	protected JPanel buildContentPanel() {
		...
		final String titleDialogMessageLabel = "上传元件库到指定服务器";
		
		//setting the title
		setTitleMessage("上传元件库");
		...
		Runnable runA = new Runnable(){
			public void run(){
				...
				try {
					...
					//下载文件
					currentName.setText("准备上传元件库");
					progressBar.setValue(20);
					pause(1);
					// 转移到FTP服务器目录至指定的目录下
					ftpClient.changeWorkingDirectory(new String(SymbolConfig.remotePath
		                    .getBytes(LOCAL_CHARSET), SERVER_CHARSET));
//					ftpClient.changeWorkingDirectory(SymbolConfig.remotePath);
					System.err.println(ftpClient.getReplyString());
					
					currentName.setText("上传元件库...");
					progressBar.setValue(30);
					pause(1);
					String fileName = "symbol.svg";
					InputStream  is = new FileInputStream(fileName);
					ftpClient.storeFile(new String(fileName.getBytes(LOCAL_CHARSET),SERVER_CHARSET), is);  
					is.close();
					
					currentName.setText("上传元件库完成");
