...
public class CopyOfUploadDialog extends TitledDialog{
			...
			public void run(){
				...
				try {
					...
					try {
						...
						File dir = new File(DrawConfig.localPath);
						...
						if(files!=null)
						{
							...
							for(String file : files)
							{
								...
								currentName.setText("上传文件:"+file);
								if((j++) == size)
								{
									progressBar.setValue(25+(i++));
									j=0;
								}
								InputStream  is = new FileInputStream(DrawConfig.localPath+File.separator+file);
								ftpClient.storeFile(new String(file.getBytes(LOCAL_CHARSET),SERVER_CHARSET), is);  
								is.close();
							}
