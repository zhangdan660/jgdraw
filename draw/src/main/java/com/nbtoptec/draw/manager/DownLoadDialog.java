...
public class DownLoadDialog extends TitledDialog{
	...
	protected JPanel buildContentPanel() {
		...
		final String titleDialogMessageLabel = "从指定服务器下载文件（*.svg）";
		
		//setting the title
		setTitleMessage("同步文件");
					...
					try {
						...
						if(files!=null)
						{
							...
							for(String file : files)
							{
								...
								currentName.setText("下载文件:"+file);
								if((j++) == size)
								{
									progressBar.setValue(25+(i++));
									j=0;
								}
								OutputStream is = new FileOutputStream("download"+File.separator+ file);
								ftpClient.retrieveFile(new String(file.getBytes(LOCAL_CHARSET),SERVER_CHARSET), is);
								is.close();
							}
