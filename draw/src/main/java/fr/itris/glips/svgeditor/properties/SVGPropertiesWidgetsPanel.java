...
public class SVGPropertiesWidgetsPanel extends JPanel {
	 ...
	 * @param tabMap
	 *            the map associating a tab id to its label
	 * @param propertyItemsMap
	 *            the map associating a tab name to a list (LinkedList) of
	 *            SVGPropertiesItems objects
	 */
	public SVGPropertiesWidgetsPanel(SVGProperties properties,
			LinkedHashMap tabMap, LinkedHashMap propertyItemsMap) {
			...
			changeListener = new ChangeListener() {
				...
				public void stateChanged(ChangeEvent arg0) {
					...
					if (tabPanel != null) {
						...
						widgetsPanel.properties.setTabId(tabPanel
								.getTitleAt(tabPanel.getSelectedIndex()));
					}
				}
			};
	...
	/**
	 * @return a JTabbedPane containing the widgets allowing to change the
	 *         properties of the node
	 */
	protected JTabbedPane getTabbedPane() {

		if (propertyItemsMap != null && propertyItemsMap.size() > 0
				&& tabMap != null) {
			...
			// for each tab
			for (Iterator it = new LinkedList(propertyItemsMap.keySet())
					.iterator(); it.hasNext();) {

				try {
					// gets the name and the label of the tab, and the list of
					// the property items linked with it
					cur = (String) it.next();
				...
				// creates the panel for the current tab and adds it to the
				// tabbed pane
	...
	/**
	 * @param propertyItemsList
	 *            the list of the property items
	 * @return a JPanel containing the widgets allowing to modify the property
	 *         values
	 */
	protected JPanel getTabPanel(LinkedList propertyItemsList) {
		...
		if (propertyItemsList != null) {
			...
			// for each property item, the linked panel is added to the tab
			// panel
	...
	/**
	 * @param propertyItem
	 *            a property item object
	 * @return an array containing a label and the widget allowing to change the
	 *         property of the given node
	 */
	protected SVGPropertiesWidget getPropertyComponents(
			SVGPropertyItem propertyItem) {
		...
		if (propertyItem != null) {
			...
			if (valueType != null && !valueType.equals("")) {
				...
				if (valueType.equals("renderchooser")) {
					...
					widget = new SVGPropertiesTwoRadioButtonsWidget(
							propertyItem);
					...
					widget = new SVGPropertiesFontSizeChooserWidget(
							propertyItem);

				} else if (valueType.equals("positivenumberchooser")
						|| valueType.equals("numberchooser")) {

					widget = new SVGPropertiesNumberChooserWidget(propertyItem);

					// a widget used to choose the preserve aspect ratio
					// attribute
				} else if (valueType.equals("preserveaspectratiochooser")) {

					widget = new SVGPropertiesPreserveAspectRatioChooser(
							propertyItem);
				} else if (valueType.equals("meas")) {
					widget = new SVGPropertiesMeasWidget(propertyItem);
				} else if (valueType.equals("href")) {
					widget = new SVGPropertiesHrefWidget(propertyItem);
				}
			}
		}
