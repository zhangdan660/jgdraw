...
public class SVGProperties extends ModuleAdapter{
	...
	public SVGProperties(Editor editor) {
		...
		Rectangle	frameBounds = editor.getPreferredWidgetBounds("properties");
		propertiesPanel.setLocation(frameBounds.x, frameBounds.y);
	...
	protected SVGPropertyItem getProperty(LinkedList nodelist, Node property){
		...
		if(property!=null){
			...
			String type="", name="", valueType="", defaultValue="", constraint="",desc="";
			...
			desc=((Element)property).getAttribute("desc");
