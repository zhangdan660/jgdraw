...
public class IOModule extends ModuleAdapter {
	...
	protected String[] ids={"FileNew", "FileOpen", 
		"FileOpenRecent", "FileSave", "FileSaveAs", 
			"FileExport", "FilePrint", "FileClose", "EditorExit", "UpLoad", "DownLoad"};
	
	protected String[] accelerators={
		"ctrl N", "ctrl O", null, "ctrl S", "ctrl shift S", 
			null, "ctrl P", "ctrl W", "Esc", null, null};
	...
	protected void handleItemsState(){
		...
		menuItems[3].setEnabled(true);  //保存
	...
	protected void doAction(int index){
		...
		switch (index){
			...
			case 9 :
				...
				Editor.getEditor().getIOManager().getUpload().doUpLoad();;
				break;
				
			case 10 :
				
				Editor.getEditor().getIOManager().getDownload().doDownLoad();;
				break;
		}
	}
}
