...
public class SameDimensionsModule extends DomActionsModule {
	...
	public SameDimensionsModule(Editor editor) {
		...
		int[] types={//SAME_SIZE, SAME_WIDTH, SAME_HEIGHT,
				SAME_WIDTH_EACH, SAME_HEIGHT_EACH,SAME_SIZE_EACH,};
		actionsTypes=types;
		
		//filling the arrays of the ids
		String[] ids={//"SameSize", "SameWidth", "SameHeight",
				 "SameWidthEach", "SameHeightEach","SameSizeEach"};
		actionsIds=ids;
		createItems();
	}
	...
	protected void doAction(
		...
		Set<Element> reference = new HashSet<Element>();
		...
		if(type==SAME_SIZE || type==SAME_WIDTH || type==SAME_HEIGHT){
			...
			if(Editor.getEditor().getCtrlSelectedElement() != null)
			{
				clipboardBounds = boundsMap.get(Editor.getEditor().getCtrlSelectedElement());
//				System.out.println(Editor.getEditor().getCtrlSelectedElement()+ "\t" + clipboardBounds);
			} else {
				//没有ctrl选中的元素，不执行后续操作
				return;
			}
			...
			for(Element element : elements){
				...
				if(Editor.getEditor().getCtrlSelectedElement() == element)
				{
					continue;
				}
	...
	protected boolean selectionCorrect(int index, Set<Element> elements) {
		...
		/*if(handle != null && elements.size() > 1)
		{
			isCorrect = true;
		}*/
