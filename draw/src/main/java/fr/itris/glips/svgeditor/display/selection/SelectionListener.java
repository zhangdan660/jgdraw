...
public class SelectionListener extends MouseAdapter implements MouseMotionListener {
	...
	public void mousePressed(MouseEvent evt) {
		...
		if(selection.getSVGHandle().getLayerManager().getEditingLayer().isLock())
		{
			SelectionDialog sd = new SelectionDialog((Frame)Editor.getParent());
			sd.showDialog(null,null);
		...
		if(selectionMode!=SelectionInfoManager.NONE_MODE && 
			...
			switch (selectionMode) {
				...
				case SelectionInfoManager.ZONE_MODE :
					...
					boolean isMultiSelectionEnabled=isMultiSelectionEnabled(evt);	//true when ctrl down
