...
public class Selection {
	...
	public Selection(final SVGHandle handle) {
		...
		setParentElement(handle.getLayerManager().getEditingLayer().getElement(), false);
	...
	public void setSelection(Point2D currentPoint, boolean isMultiSelection) {
		...
		//add by zd
		//record the last selected element by ctrl+mouse 
		if(isMultiSelection)
		{
			Editor.getEditor().setCtrlSelectedElement(element);
		} else {
			Editor.getEditor().setCtrlSelectedElement(null);
		}
	}
	...
	public void handleSelection(
		...
		//getting all the elements that can be found in the given rectangle in the canvas
         ...
         elements = handleLayerSelection(rectangle, scaledRectangle, elements);
	...
	public Set<Element> handleLayerSelection(Rectangle2D rectangle,Rectangle2D scaledRectangle, Set<Element> elements)
	{
		//getting all the elements that can be found in the given rectangle in the canvas
		Rectangle2D r2=null;
		...
		ArrayList<SVGLayer> layerlist = svgHandle.getLayerManager().getLayerlist();	//获取所有图层
		for(SVGLayer layer : layerlist)
		{
			if(layer.isLink() && !layer.isLock() )
			{
				for(Node current=layer.getElement().getFirstChild(); 
						current!=null; 
						current=current.getNextSibling()){
	...
	public void selectAllElements(){
		...
		elementsToSelect = selectLayerElements(elementsToSelect);
		
		handleSelection(elementsToSelect, true, true);
	}
	
	/** 
	 * add Element from Layer linked
	 * @param elementsToSelect
	 * @return
	 */
	public Set<Element> selectLayerElements(Set<Element> elementsToSelect)
	{
		ArrayList<SVGLayer> layerlist = svgHandle.getLayerManager().getLayerlist();	//获取所有图层
		for(SVGLayer layer : layerlist)
		{
//			System.out.println(layer.isLock()+"\t" + layer.isVisible()+"\t" + layer.isEdit()+"\t"+layer.isLink()+"\t" + layer.getName());
			if(layer.isLink() && !layer.isLock())	//非当前图层的链接图层
			{
				//computing the set of the elements to be selected
				for(Node node=layer.getElement().getFirstChild(); 
					node!=null; node=node.getNextSibling()){
		...
		if(! actionDisabled && ! isSelectionLocked()){
			...
			if(firstPoint==null) {
				...
				if(actionItem==null) {
					...
					if(translationElement == null)		//getting the element from other layer
					{
						ArrayList<SVGLayer> layerlist = svgHandle.getLayerManager().getLayerlist();	//获取所有图层
	            		for(SVGLayer layer : layerlist)
	            		{
	            			if(!layer.isEdit() && layer.isLink() && !layer.isLock())		//非当前编辑层
	            			{
	            				translationElement = svgHandle.getSvgElementsManager().
	            						getNodeAt(layer.getElement(), scaledPoint);
	...
	public void handleCursor(Point currentMousePoint) {
        ...
        if(currentItem!=null){
        	...
        	if(selectionMode==SelectionInfoManager.DRAWING_MODE ||
                ...
                if(element  == null)		//跨图层选中操作
                {
                	ArrayList<SVGLayer> layerlist = svgHandle.getLayerManager().getLayerlist();	//获取所有图层
            		for(SVGLayer layer : layerlist)
            		{
            			if(!layer.isEdit() && layer.isLink() )		//非当前编辑层
            			{
            				element = svgHandle.getSvgElementsManager().
            	                	getNodeAt(layer.getElement(), scaledPoint);
