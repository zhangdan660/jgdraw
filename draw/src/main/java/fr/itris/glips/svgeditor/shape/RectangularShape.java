...
public abstract class RectangularShape extends AbstractShape {
	...
	public UndoRedoAction resize(
		...
		*/	handle.getSvgElementsManager().getTransform(element);
		...
		SelfAffineTransform selftr = handle.getSvgElementsManager().getSelfTransform(element);
		if(!(element instanceof SVGOMUseElement))
		{
			selftr.preferiosTransform = selftr.currentTransform;
		}
		
		final AffineTransform initialTransform = selftr.preferiosTransform;
		
		if(initialTransform.isIdentity()){
			
			//getting the current element bounds
			final java.awt.geom.RectangularShape shape=
				(java.awt.geom.RectangularShape)getShape2(handle, element, false);
...
//			handle.getSvgElementsManager().getTransform(element);
		...
		final AffineTransform previousElementTransform = selftr.preferiosTransform;
...
//			handle.getSvgElementsManager().getTransform(element);
	...
	public CanvasPainter showTranslateAction(SVGHandle handle, 
		...
		Shape shape=getShape2(handle, element, true);
...
//			transform = handle.getSvgElementsManager().getSelfTransform(element).preferiosTransform;
...
//		System.out.println(this.getClass().getName()+"\t"+transform);
	...
	public CanvasPainter showAction(
		...
		if(canPaintShape){
			...
			if(useTranform && actionTransform!=null){
				...
				shape=getShape2(handle, element, true);
				...
				if(element instanceof SVGOMUseElement)
				{
					...
					shape=handle.getTransformsManager().
							getScaledShape(shape, false, new AffineTransform());
				} else {
	...
	@Override
	public Shape getShape2(SVGHandle handle, Element element, boolean isOutline) {
		
		RoundRectangle2D rectangle=null;
		
		/*if( !(element instanceof SVGOMUseElement || element instanceof SVGAElement))
		{
			Rectangle2D bound = handle.getSvgElementsManager().getNodeGeometryBounds(element);
			
			//creating the rectangle
			rectangle=new RoundRectangle2D.Double(bound.getX(), bound.getY(), bound.getWidth(), bound.getHeight(), 0, 0);
	...
	public Set<SelectionItem> getRadiusSelectionItems(
		...
		RoundRectangle2D rect=(RoundRectangle2D)getShape2(handle, element, false);
		...
		if(element instanceof SVGOMUseElement)
		{
			SelfAffineTransform saf = handle.getSvgElementsManager().getSelfTransform(element);
//			transform.concatenate(saf.preferiosTransform);
