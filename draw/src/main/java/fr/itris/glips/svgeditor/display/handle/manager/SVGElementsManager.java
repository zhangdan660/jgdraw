...
import org.apache.batik.gvt.SelfAffineTransform;
		...
		if(shape!=null){
			...
			if(ctxt!=null){
				...
				if(gnode!=null){
					...
					try{
						if(shape instanceof SVGOMUseElement)
						{
							SelfAffineTransform saf = gnode.getSelfTransform();
							AffineTransform transform = new AffineTransform(saf.currentTransform);
				...
				if(gnode!=null){
					...
					if(bounds2D ==null) {
						...
						if( shape instanceof SVGOMUseElement)
						{
							...
							bounds2D = gnode.getGeometryBounds(saf.currentTransform);
							...
							bounds2D=gnode.getGeometryBounds();
							if(bounds2D == null)
							{
								bounds2D =  new Rectangle2D.Double(1,1,1,1);
							}
...
//		System.out.println("SVGElemengsManger\t"+bounds);
		return bounds;
	}
	...
	/**
	 * parses and returns the transform that is described 
	 * in the transform attribute of the element
	 * @param shapeElement an element
	 * @return the element's transform
	 */
	public AffineTransform parseTransform(Element shapeElement){
		
		return AWTTransformProducer.createAffineTransform(
				shapeElement.getAttribute("transform"));
	}
	
	/**
	 * returns the affine transform that is applied to the given element
	 * @param shapeElement a shape element
	 * @return the affine transform that is applied to the given element, that is never null
	 */
	 ...
	 * @return the affine transform that is applied to the given element, that is never null
	 */
	public SelfAffineTransform getSelfTransform(Element shapeElement){

		AffineTransform af=null;
		SelfAffineTransform saf =  null;
		...
		if(shapeElement!=null){
			...
			if(ctxt!=null){
				...
				if(gnode!=null){
					...
					saf = gnode.getSelfTransform();
					af = saf.currentTransform;
	...
	public Element getNodeAt(Node parent, Point2D point){
		...
		if(parent!=null && point!=null){
			...
			Rectangle2D zone=new Rectangle2D.Double(
				point.getX()-diff, point.getY()-diff, 2*diff, 2*diff);
					...
					for(int i=childNodes.getLength()-1; i>=0; i--){
						...
						if(node!=null && node instanceof Element){
							...
							if(EditorToolkit.isElementAShape(element)){
								...
								if(intersects(gnode, zone, element)){
	...
	public boolean intersectUse(GraphicsNode gnode, Rectangle2D zone, Element element)
	{
		if(gnode!=null){
			...
			if(gnode.getTransform()!=null){
				...
				sensitiveBounds=gnode.getTransform().
					createTransformedShape(sensitiveBounds).getBounds2D();
			}
			...
			if(Editor.isDebug())
				System.out.println(sensitiveBounds.intersects(zone) + "\t" + gnode +"\t"+ sensitiveBounds  + "\t" + zone);

			if(sensitiveBounds.intersects(zone))
			{
				return true;
			}
		}
	...
	/**
	 * to check gnode before invoke (GraphicsNode gnode, Rectangle2D zone)
	 * @param gnode
	 * @param zone
	 * @param elment
	 * @return whether the graphics node sensitive bounds intersects the provided zone
	 * @author zhangdan
	 */
	protected boolean intersects(GraphicsNode gnode, Rectangle2D zone, Element element) {
		...
		if(element instanceof SVGOMGElement)
		{
			...
			for (int i=childNodes.getLength()-1; i>=0; i--)
			{
				...
				{
					intersects = intersectUse(gnode, zone, node);
					return  intersects;
				} else if(node instanceof SVGOMGElement)
				{
//					System.out.println(element);
					intersects = intersects(gnode, zone, node);
					return intersects;
				}
			}
		}
		...
		{
			intersects = intersectUse(gnode, zone, element);
		} else {
			
			intersects = intersects(gnode, zone);
	...
	/**
	 * returns whether the graphics node sensitive bounds intersects the provided zone
	 * @param gnode a gnode
	 * @param zone the zone
	 * @return whether the graphics node sensitive bounds intersects the provided zone
	 */
		...
		if(gnode!=null){
			...
			if(sensitiveBounds!=null && sensitiveBounds.intersects(zone)){
				...
				if(gnode instanceof ShapeNode){
					...
					//checking if one of the children of the composite node intersects the zone
	...
	/**
	 * find the accurate id for a node, that does not exist for the 
	 * nodes in the handle document, or in the provided elements set
	 * @param baseString the base of the id
	 * @param excludedNodes the set of the elements that should be 
	 * taken into account for checking the uniqueness of the id
	...
	public String getId(String baseString, Set<Element> excludedNodes){
		...
		if(doc!=null){
			...
			//adding to the list all the ids found among the children of the root element
	...
	/**
	 * @return the list of the ids of the shape nodes that are 
	 * contained in the given svg document
	 */
