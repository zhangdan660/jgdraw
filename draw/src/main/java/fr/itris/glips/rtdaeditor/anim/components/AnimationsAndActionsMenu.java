...
//zd
	...
	/**
	 * builds the menu items  create by zd
	 * @param specDocument the specification 
	 * document used to build the menu items
	 * @param optionalBundle the bundle to use 
	 * when a label can't be found in the default bundle
	 * @param selectedElement the element that is selected on the canvas
	 */
	protected void buildMenuItems2(Document specDocument, 
			ResourceBundle optionalBundle, final Element selectedElement) {
		...
		if(specDocument!=null) {
			...
			//getting all the group
			NodeList groups=rtdaAnimationsDocument.
				getDocumentElement().getElementsByTagName("animationgroup2");
			for(int j=0; j<groups.getLength(); j++)
			{
				
				Element group=(Element)groups.item(j) ;
				NodeList nodes=group.getElementsByTagName("animation");
				String label="";
				
				for(int i=0; i< nodes.getLength(); i++)
				{
					final Element parent=(Element)nodes.item(i);
					label=parent.getAttribute("name");
					try{
						label=getLabel(label, optionalBundle); 
					}catch (Exception ex) {}
					...
					final ActionListener actionListener=new ActionListener() {
						...
						public void actionPerformed(ActionEvent evt) {
							...
							//creating the animation
							createAnimationOrAction2(parent, selectedElement);
						}
					};
	...
	/** 
	 * modified by zd  replace rtda with SMIL because rtda is not supported by chrome
	...
	protected void createAnimationOrAction(
		...
		if(specElement!=null && selectedElement!=null) {
			...
			//creating the new node and its attributes  modified by zd
			final Element newElement=selectedElement.getOwnerDocument().
					createElementNS(fr.itris.glips.rtda.AnimationsToolkit.rtdaNameSpace, 
	 ...
	 * @param selectedElement the element that is selected on the canvas
	 */
	protected void createAnimationOrAction2(
			Element specElement, final Element selectedElement) {
		...
		if(specElement!=null && selectedElement!=null) {
			...
			//creating the new node and its attributes  modified by zd
			final Element newElement=selectedElement.getOwnerDocument().
					createElementNS(selectedElement.getOwnerDocument().getNamespaceURI(), 
							specElement.getAttribute("name"));
	...
	public void createPopUpMenu(Document specDocument, 
		...
		buildMenuItems2(specDocument, optionalBundle, selectedElement);
